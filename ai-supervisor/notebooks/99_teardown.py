...
# MAGIC
# MAGIC The **catalog is NEVER dropped**. Refuses to run if `target_schema` is empty or the
# MAGIC catalog/schema is in `never_touch`. Resets `deployment_manifest.json` on success.
# MAGIC
...
for env, a in manifest_get("genie", {}).items():
    sid = a.get("space_id") if isinstance(a, dict) else None
    if not sid:
        continue
    if sid in nt:
        print(f"  (skip) {env} {sid} in never_touch")
        continue
    try:
        w.api_client.do("DELETE", f"/api/2.0/data-rooms/{sid}")
        ok(f"deleted Genie space {env} {sid}")
    except Exception as e:
...
print(f"\nTeardown: removed app/lakebase/genie/dashboard + schema {cat}.{sch}")
