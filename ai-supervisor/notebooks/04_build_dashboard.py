...
# MAGIC Reads the **shipped** dashboard definition (`artefacts/dashboard.json`), remaps every
# MAGIC origin-schema reference to your target, points its warehouse at the target, and publishes it.
...
NAME = "Ramsay Health — Group Operations (5 Hospitals)"

w = target_sql().w
if not ARTEFACT.exists():
    fail(f"shipped dashboard definition missing: {ARTEFACT}")
_art = json.loads(ARTEFACT.read_text())
src_def = _art.get("serialized_dashboard") or ""
if not src_def:
